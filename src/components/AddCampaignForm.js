...
const AddCampaignForm = () => {
  ...
  const handleChange = (e) => {
    ...
    // Handle other fields (including userName)
      ...
      if (name === "userName") {
        console.log(`userName field changed to: "${value}"`);
      }
    }
  ...
  const handleSubmit = (e) => {
    ...
    console.log(
      "Users loaded:",
      users && users.length > 0 ? `Yes (${users.length} users)` : "No"
    );
    ...
    if (formData.userName && formData.userName.trim()) {
      ...
      console.log(`Looking for user with name: "${trimmedUserName}"`);
      ...
      if (users && users.length > 0) {
        ...
        if (user) {
          userId = user.id;
          console.log(`✓ Found user in API: "${user.name}" with id: ${userId}`);
        } else {
          ...
          userId = newUserId;
          console.log(`✗ User "${trimmedUserName}" not found in API.`);
          console.log(
            `  Auto-generated new userId: ${userId} (will show "Unknown User")`
          );
          console.log(
            `  Note: Custom user names are allowed but will display as "Unknown User" since they don't exist in the API.`
          );
        }
      } else {
        // Users not loaded yet - auto-generate userId
        console.warn(
          "Users not loaded yet. Auto-generating userId for custom name."
        );
        const currentCampaigns = store.getState().campaigns.campaigns;
        const existingUserIds = new Set(
          currentCampaigns
        ...
        userId = newUserId;
        console.log(
          `Auto-generated userId: ${userId} (will show "Unknown User")`
        );
      }
    } else {
      // No userName provided - will be handled by AddCampaigns function
      console.log(
        "No userName provided, userId will be auto-generated by AddCampaigns function"
      );
    }

    console.log(`Final userId for campaign: ${userId}`);
